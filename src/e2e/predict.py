...
def create_packet_arrival_window(packet_arrival_events, arrival_event, history_window_size):
    # find the index of the arrival_event in the packet_arrival_events
    idx = packet_arrival_events.index(arrival_event)
    if idx < history_window_size:
        return None
    events_window = []
    for pos, event in enumerate(packet_arrival_events[idx-history_window_size+1:idx+1]):
...
def create_link_quality_window(sorted_link_events, arrival_event, dim_process_no_mcs, history_window_size):

    # find the latest event before the arrival_event['timestamp']
    idx = 0
    ...
    events_window = []
    for pos, event in enumerate(sorted_link_events[idx-history_window_size+1:idx+1]):
        idx_event = pos
...
def create_scheduling_window(sorted_scheduling_events, arrival_event, history_window_size, num_post_scheduling_events):
    ...
    events_window = []
    # idx + 1 since the first schedule event must be included as the label
    idx_pre = idx+1
    ...
    # sort the events_window_v3 based on timestamp as it is reveresed
    events_window = sorted(events_window, key=lambda x: x['timestamp'], reverse=False)
    # remove 'timestamp' field from the events_window
    for pos, event in enumerate(events_window):
        del event['timestamp']
        event['idx_event'] = pos
    ...
    while idx_post < len(sorted_scheduling_events) and len(post_scheduling_events) < num_post_scheduling_events:
        ...
        post_scheduling_events.append(event)
        idx_post += 1
...
def e2e_delay_prediction(dataset, arrival_model_runner, link_quality_runner, scheduling_runner, exp_config, dim_process_no_mcs, mcs_to_bytes_table, max_num_segments=5, num_packets=1):
    ...
    # we assume 'mcs_index' won't change during a packet transmission
    packet_len = dataset[0]['history']['scheduling'][-2]['len']-3
    mcs_index = dataset[0]['history']['scheduling'][-2]['mcs_index']
    num_symbols = dataset[0]['history']['scheduling'][-2]['num_symbols']
    prev_segment_time_since_start = dataset[0]['history']['scheduling'][-2]['time_since_start']
    prev_segment_slot = dataset[0]['history']['scheduling'][-2]['slot']
    prev_segment_idx = dataset[0]['history']['scheduling'][-2]['idx_event']
    prev_link_time_since_start = dataset[0]['history']['link_quality'][-2]['time_since_start']
    prev_link_idx = dataset[0]['history']['link_quality'][-2]['idx_event']

    # start scheduling prediciton for the segments
    scheduling_source_data = [dataset[0]['history']['scheduling']]
    link_source_data = [dataset[0]['history']['link_quality']]
    predicted_segments = []
    sum_departed_bytes = 0
    segment_num = 1
    for _ in range(max_num_segments):
        # get the segment's time
        ...
        segment_dtime = np.mean(cp_dtime_samples[:, 0, 0])
        segment_num_rbs = np.mean(cp_num_rbs_samples[:, 0, 0])
        segment_time_since_start = (prev_segment_time_since_start + segment_dtime) % max_num_frames*num_slots_per_frame*slots_duration_ms
        segment_slot = (prev_segment_slot + int(segment_dtime/slots_duration_ms)) % num_slots_per_frame
        segment_len_bytes = mcs_to_bytes_table[mcs_index]*segment_num_rbs*num_symbols

        # next we make a link quality prediction
        ...
        link_dtime = cp_dtime_samples[0,0]
        link_type_pred = cp_type_samples[0,0]
        link_time_since_start = (prev_link_time_since_start + link_dtime) % max_num_frames*num_slots_per_frame*slots_duration_ms
        
        # start filling a predicted segment for the next segment
        # if link_time_since_start is close enough to time_since_start e.g. 2ms
        # we assume the event happens
        if abs(link_time_since_start - segment_time_since_start) < 2 and link_type_pred < dim_process_no_mcs:
            ...
            segment_mac_retx = link_type_pred % 4
            segment_rlc_failed = int(np.floor(link_type_pred / 4))
            # consider this a retransmission event
            predicted_link_quality_event = {
                'idx_event' : prev_link_idx + 1,
                'type_event' : int((segment_mac_retx-1)+(int(segment_rlc_failed)*4)),
                'time_since_start' : link_time_since_start,
                'time_since_last_event' : link_dtime,
                'mcs_index' : mcs_index
            }
            link_source_data.append(predicted_link_quality_event)
        else:
            segment_mac_retx = 0
            segment_rlc_failed = 0
            predicted_link_quality_event = {}

        predicted_segment = {
            'idx_event' : prev_segment_idx + 1,
            'type_event': segment_num,
            'slot' : segment_slot,
            'len' : segment_len_bytes,
            'mcs_index' : mcs_index,
            'mac_retx' : segment_mac_retx,
            'rlc_failed' : segment_rlc_failed,
            'num_rbs' : segment_num_rbs,
            'num_symbols' : num_symbols,
            'time_since_start' : segment_time_since_start,
            'time_since_last_event' : segment_dtime
        }
        predicted_segments.append(predicted_segment)
        scheduling_source_data.append(predicted_segment)
        sum_departed_bytes += ( segment_len_bytes if not segment_rlc_failed else 0 )

        # break when we have transmitted the packet
        if sum_departed_bytes >= packet_len:
            break

        segment_num += 1
        prev_segment_time_since_start = segment_time_since_start
        prev_segment_slot = segment_slot
        prev_segment_idx = prev_segment_idx + 1
        prev_link_time_since_start = link_time_since_start if predicted_link_quality_event is not {} else prev_link_time_since_start
        prev_link_idx = prev_link_idx + 1

    return predicted_segments
    #result = arrival_model_runner.run(
    #    batch_size=1,
    #    source_data=[dataset[0]['history']['packet_arrival']],
    #    return_predictions=True
    #)
    #print(result)
...
def generate_predictions(args):
    ...
    for packet_arrival_events, link_retransmission_events, link_mcs_events, scheduling_events, time_bound in \
        ...
        for arrival_event in packet_arrival_events:
            packet_arrival_window = create_packet_arrival_window(packet_arrival_events, arrival_event, arrival_window_size)
            link_quality_window = create_link_quality_window(link_quality_events, arrival_event, dim_process_no_mcs, link_window_size)
            scheduling_window, post_scheduling_events = create_scheduling_window(scheduling_events, arrival_event, scheduling_window_size, num_post_scheduling_events)
            if packet_arrival_window and link_quality_window and scheduling_window:
                # we can make a prediction
                dataset.append({
                    'label': post_scheduling_events,
                    'history': {
                        'packet_arrival' : packet_arrival_window,
                        'link_quality' : link_quality_window,
                        'scheduling' : scheduling_window
                    }
                })
    ...
    predicted_segments = e2e_delay_prediction(
        dataset, 
        arrival_model_runner, 
        link_quality_runner, 
        scheduling_runner, 
        exp_config, 
        dim_process_no_mcs, 
