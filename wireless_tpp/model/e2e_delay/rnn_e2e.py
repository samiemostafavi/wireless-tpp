...
class RecurrentE2ESingle(TorchBaseModel):
    """Torch implementation of MDN Learning of Temporal Point Processes
    """
    ...
    def __init__(self, model_config):
        ...
        """
        super(RecurrentE2ESingle, self).__init__(model_config)
    ...
    def forward(self, seq_obj : SequenceSeperate, phase=None):
        ...
        all_preds = torch.stack([ rnn_out[:, -1, :] ], dim=1)
        mdn_params = self.mdn_head(all_preds)

        batch_size, tgt_seq_len = seq_obj.tgt_non_pad_mask.float().size() # we predict only 1 step here
