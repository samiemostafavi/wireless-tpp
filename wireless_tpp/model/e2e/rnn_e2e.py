...
class RecurrentE2E(TorchBaseModel):
    ...
    def __init__(self, model_config):
        ...
        if self.bidirectional:
            self.linear_layer_input_size = 2 * self.d_model
        else:
        ...
        if self.last_layer_mlp:
            ...
            self.linear = nn.Sequential(
                nn.Linear(self.linear_layer_input_size, self.d_model * 2),
                nn.Linear(self.d_model * 2, self.tgt_seq_len * 3 * self.num_mix_components)
            )
        else:
    ...
    def get_pred_distribution(self, rnn_out, step=False) -> NormalMixtureDistribution:
        ...
        """
        if self.last_layer_mlp:
            assert rnn_out.dim() == 2
            assert rnn_out.size(1) == self.linear_layer_input_size
            ...
            assert rnn_out.dim() == 3
            if step:
                assert rnn_out.size(1) == 1
            else:
                assert rnn_out.size(1) == self.tgt_seq_len
            assert rnn_out.size(2) == self.linear_layer_input_size
    ...
    def loglike_loss_tf(self, batch, forward=True):
        ...
        pred_dist = self.get_pred_distribution(tgt_rnn_out, step=False)  # Predict based on RNN output
    ...
    def get_rnn_input_step_old(self, rnn_out_step, interarrival_time):
        # rnn_out: [batch_size, 1, d_model]
        # interarrival_time: [batch_size, 1]
        pred_dist_step = self.get_pred_distribution(rnn_out_step, step=True)
    ...
    def loglike_loss_eval(self, batch):
        ...
        if self.last_layer_mlp:
            ...
            pred_dist = self.get_pred_distribution(rnn_out[:, -1, :], step=False)
        else:
            ...
            pred_dist = self.get_pred_distribution(predictions, step=False)
