...
# For each window length, plot over
for idw, w_length in enumerate(categories['Window Length']):
    ...
    for idx, model_type in enumerate(categories['Type']):
        ...
        if x_vals and y_vals:
            plt.plot(x_vals, y_vals, marker=markers[idx % len(markers)], linestyle=('--' if (idw%2 == 1) else '-'), label=type_labels[model_type]+f" L={w_length}")
...
plt.tight_layout()
plt.savefig(figures_path / f"loglike_plot_w_combined.pdf")
plt.close()
